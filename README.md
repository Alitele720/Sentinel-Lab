...
- 管理后台：提供仪表盘、告警列表、规则管理、配置管理、黑名单与端口阻断管理。
- 安全入口：后台支持 IP 白名单、账号密码登录、可信代理 IP 解析和实验接口暴露开关。
- 数据导出：支持告警 CSV 导出。
- 自动化测试：`tests/test_app.py` 覆盖核心检测、后台访问控制、配置、统计接口和端口扫描抓包解析。
...
`/lab/portscan` 和 `/api/connection-events` 已从当前代码中移除。端口扫描告警来自真实 TCP SYN 抓包或测试代码直接写入的连接事件。
...
- 确认文档中的命令、路由和环境变量与当前代码一致。

当前 `.gitignore` 已覆盖 `.env`、`data/`、`*.db`、`*.log`、`__pycache__/` 和 `*.zip` 等常见敏感文件与运行产物。

## License

当前项目未声明开源许可证。如需复用、分发或二次开发，请联系作者或补充合适的许可证文件。
